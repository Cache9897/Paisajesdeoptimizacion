...
for j=1:length(x1)
    ...
    f1(j)=min((x1(j)-mu1)^2,d*1+s_1*(x1(j)-mu2_1)^2);
    
    % For 2-dimensional plotting
    for i=1:length(x2)
        fn(i)=min((x1(j)-mu1)^2+(x2(i)-mu1)^2,d*1+s_2*((x1(j)-mu2_2)^2+(x2(i)-mu2_2)^2));
    end
...
% ylabel('x2');
% zlabel('f(x1,x2)');
% title('LUNACEK BI-SPHERE FUNCTION');
% hold on;
imagesc(x1,x2,z);

f = imread ('LUNACEKbisphere.jpg');
k = rgb2gray(f);
figure(2)
imshow(k);